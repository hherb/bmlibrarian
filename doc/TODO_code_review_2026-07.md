...
- [ ] **Queue task-claiming race** — `agents/queue_manager.py:487-523`: SELECT-then-UPDATE dequeue is not atomic across processes (the `threading.Lock` only covers one process, but the class tracks `process_id` and has `recover_stuck_tasks()` for multi-process use). Two workers can claim the same task. Fix: single `UPDATE … WHERE id = (SELECT … ORDER BY priority DESC, created_at ASC LIMIT 1) RETURNING *`, or `BEGIN IMMEDIATE`.
- [ ] **Signal-handler deadlock** — `agents/queue_manager.py:88,101-103,175-196`: the SIGINT/SIGTERM handler acquires the same non-reentrant lock the interrupted code may already hold → Ctrl-C can hang any interactive CLI mid-enqueue. Also `signal.signal()` raises `ValueError` if a `QueueManager` is ever constructed off the main thread (e.g. Qt worker). Don't take the lock in the handler; only register handlers on the main thread.
- [ ] **ThesaurusExpander pool leak** — `thesaurus/expander.py:120-136`: constructs a new `DatabaseManager()` (a whole new connection pool + migration check) per uncached term on the query hot path, never closed; up to 50 terms per query → connection exhaustion. Use `get_db_manager()` like every other module.
- [ ] **Connections used after return-to-pool** — `importers/pubmed_bulk_importer.py:1048-1127` (conn passed to `_store_transparency_metadata_batch` after its `with` block exits; the `if 'conn' in dir()` guard is dead code) and `importers/pdf_matcher.py:136-137` (PDFManager keeps a returned-to-pool conn for its whole life, incl. commits/rollbacks). Use `PersistentConnection` (`database.py:242`) or restructure to per-operation acquisition.
...
- [ ] **Class-level source-ID cache** — `database.py:34-36,147-171`: `_source_id_cache`/`_source_ids` are class attributes; a second `DatabaseManager` pointed at a different database silently reuses the first database's source IDs → wrong/empty search results. Key the cache per-conninfo or make it instance-level.
