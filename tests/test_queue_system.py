...
class TestQueueAwareAgents:
    ...
    def test_scoring_agent_queue_submission(self, mock_chat):
    ...
    @patch('bmlib.llm.client.LLMClient.chat')
    def test_scoring_agent_fallback_processing(self, mock_chat):
        """Test DocumentScoringAgent fallback when no orchestrator."""
        # Mock LLM responses
        mock_chat.return_value = BmlibResponse(
            content='{"score": 3, "reasoning": "Test reasoning"}',
            model="test",
            input_tokens=10,
            output_tokens=5
        )
    ...
    @patch('bmlib.llm.client.LLMClient.chat')
    def test_agent_task_submission_methods(self, mock_chat):
    ...
    @patch('bmlib.llm.client.LLMClient.chat')
    def test_agent_without_orchestrator(self, mock_chat):
